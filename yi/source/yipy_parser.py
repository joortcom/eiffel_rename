...
# @generated by pegen from source/yipy.gram
...
""" * n_line + ' ' * (t.start[1] - col_offset) + t.string
        ...
        res = m.body[0].value
        # print(type(res))  <class 'ast.Constant'>
        return res
...
class PythonParser(Parser):
    ...
    def class_def_raw(self) -> Optional[ast . ClassDef]:
        # class_def_raw: invalid_class_def_raw | 'class' NAME ['(' bases? ')'] &&':' block
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
    ...
    @memoize
    def base(self) -> Optional[ast . Call]:
        # base: atom ['(' renames? ')']
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (base_class_name := self.atom())
            and
            (b := self._tmp_38(),)
        ):
            tok = self._tokenizer.get_last_non_whitespace_token()
            end_lineno, end_col_offset = tok.end
            return ast . Call ( func = base_class_name , args = None , keywords = b if b else [] , lineno=start_lineno, col_offset=start_col_offset, end_lineno=end_lineno, end_col_offset=end_col_offset , )
        self._reset(mark)
        return None

    @memoize
    def renames(self) -> Optional[Any]:
        # renames: ','.rename+
        mark = self._mark()
        if (
    ...
    @memoize
    def rename(self) -> Optional[Any]:
        # rename: 'rename' NAME ['as' NAME]
        mark = self._mark()
        tok = self._tokenizer.peek()
        start_lineno, start_col_offset = tok.start
        if (
            (literal := self.expect('rename'))
            and
            (a := self.name())
            and
            (b := self._tmp_41(),)
    ...
    def with_item(self) -> Optional[ast . withitem]:
        ...
        if (
            ...
            (t := self.star_target())
            and
            self.positive_lookahead(self._tmp_66, )
        ):
    ...
    def invalid_class_def_raw(self) -> Optional[NoReturn]:
        # invalid_class_def_raw: 'class' NAME ['(' bases? ')'] ':' NEWLINE !INDENT
        mark = self._mark()
        if (
            (a := self.expect('class'))
            and
    ...
    @memoize
    def _loop1_163(self) -> Optional[Any]:
    ...
    def _loop0_164(self) -> Optional[Any]:
        # _loop0_164: lambda_param_no_default
        mark = self._mark()
        children = []
        while (
            (lambda_param_no_default := self.lambda_param_no_default())
    ...
    def _loop0_170(self) -> Optional[Any]:
        # _loop0_170: ',' (expression ['as' star_target])
        mark = self._mark()
        children = []
        while (
            (literal := self.expect(','))
    ...
    def _gather_169(self) -> Optional[Any]:
        # _gather_169: (expression ['as' star_target]) _loop0_170
        mark = self._mark()
        if (
            (elem := self._tmp_203())
            is not None
    ...
    def _loop0_172(self) -> Optional[Any]:
        # _loop0_172: ',' (expressions ['as' star_target])
        mark = self._mark()
        children = []
        while (
            (literal := self.expect(','))
    ...
    def _gather_171(self) -> Optional[Any]:
        # _gather_171: (expressions ['as' star_target]) _loop0_172
        mark = self._mark()
        if (
            (elem := self._tmp_204())
            is not None
    ...
    def _loop0_174(self) -> Optional[Any]:
        # _loop0_174: ',' (expression ['as' star_target])
        mark = self._mark()
        children = []
        while (
            (literal := self.expect(','))
    ...
    def _gather_173(self) -> Optional[Any]:
        # _gather_173: (expression ['as' star_target]) _loop0_174
        mark = self._mark()
        if (
            (elem := self._tmp_205())
            is not None
    ...
    def _loop0_176(self) -> Optional[Any]:
        # _loop0_176: ',' (expressions ['as' star_target])
        mark = self._mark()
        children = []
        while (
            (literal := self.expect(','))
    ...
    def _gather_175(self) -> Optional[Any]:
        # _gather_175: (expressions ['as' star_target]) _loop0_176
        mark = self._mark()
        if (
            (elem := self._tmp_206())
            is not None
    ...
    def _loop0_185(self) -> Optional[Any]:
        # _loop0_185: ',' double_starred_kvpair
        mark = self._mark()
        children = []
        while (
            (literal := self.expect(','))
    ...
    def _gather_184(self) -> Optional[Any]:
        # _gather_184: double_starred_kvpair _loop0_185
        mark = self._mark()
        if (
            (elem := self.double_starred_kvpair())
            is not None
    ...
    KEYWORDS = ('False', 'None', 'True', 'and', 'as', 'assert', 'async', 'await', 'break', 'class', 'continue', 'def', 'del', 'elif', 'else', 'except', 'finally', 'for', 'from', 'global', 'if', 'import', 'in', 'is', 'lambda', 'nonlocal', 'not', 'or', 'pass', 'raise', 'rename', 'return', 'try', 'while', 'with', 'yield')
